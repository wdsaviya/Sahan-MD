...
try {
    ...
    console.log("📡Connected to the DARK-SHAN-MD SERVER")
} catch {
...
let cc = Config.sessionName.replace(/DARKSHAN;;;/g, "");
...
await updateStatus()
 ...
 Void.ev.on('group-participants.update', async(anu) => {
        ...
        Void.ev.on('connection.update', async(update) => {
                ...
                if (connection === 'open') {
                    console.log("✅ DARK SHAN MD Login Successful!");
                    ...
                    for (let i = 0; i < check.length; i++) {
                     ...
                     console.log("✅  DARK SHAN MD External Plugins Installed!");fs.readdirSync(__dirname + "/../commands").forEach((plugin) => {
